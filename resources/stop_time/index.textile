...
The *arrival_time* specifies the arrival time at a specific stop for a specific trip on a route. The format of this field is described in the "Time Format":#format section.

h4. departure_time %(required)%

The *departure_time* specifies the departure time from a specific stop for a specific trip on a route. The format of this field is described in the "Time Format":#format section.

h4. stop_id %(required)%

The *stop_id* field contains an ID that uniquely identifies a stop. Multiple routes may use the same stop. The *stop_id* is referenced from the "stops.txt":stop file. If *location_type* is used in stops.txt, all stops referenced in stop_times.txt must have *location_type* of 0.

Where possible, *stop_id* values should remain consistent between feed updates. In other words, stop A with *stop_id* 1 should have *stop_id* 1 in all subsequent data updates.
...
The units used for *shape_dist_traveled* in the *stop_times*.txt file must match the units that are used for this field in the "shapes.txt file":shape.

h3(#format). Time Format
