...
This project is currently a sub-component of the "KnowTime Server":knowtime_git project, but hopefully can of use to others projects. This project also depends on another GitHub project, "GTFS Reader":gtfs_reader_git, to actually process the __.zip__ files in which GTFS feeds are contained.
...
That being said, the source data is only used when updating the data sources. As this is most often done via @rake@ tasks (see below), you may want to restrict your configuration to that.
...
[knowtime_git]{{ site.data.urls.knowtime_git }}
